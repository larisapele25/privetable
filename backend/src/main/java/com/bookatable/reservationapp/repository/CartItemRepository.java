...
public interface CartItemRepository extends JpaRepository<CartItem, Long> {
    ...
    @Query("SELECT c FROM CartItem c WHERE c.product.restaurant.id = :restaurantId AND DATE(c.addedAt) = :date")
    List<CartItem> findByRestaurantIdAndDate(@Param("restaurantId") Long restaurantId, @Param("date") LocalDate date);
